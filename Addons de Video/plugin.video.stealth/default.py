...
INTRO = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.andybuildstv/intro.mp4'))
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.@ndybuilds Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(@ndy builds,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(@ndy builds,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
################################
				
def SKindex():
	addon_log("SKindex")
	addDir3('[COLOR aqua]latest movies[/COLOR]','http://watchmovies-online.org/last-movies-added/',54,'http://www.latestmovierelease.in/wp-content/uploads/2017/01/latest-movies-releases-logo.png',FANART,'')
	addDir3('[COLOR aqua]ALL MOVIES[/COLOR]','http://watchmovies-online.org/movie-index/',54,'https://lifeplunge.com/wp-content/uploads/2016/09/movies-logo.jpg',FANART,'')
	addDir3('[COLOR aqua]TV SERIES[/COLOR]','http://watchseries-online.pl/index',54,'https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcRn79lvbrgozWtLAKpGzyt0u3bKsRGbeGO1aIhGTsyaERz4-rsLew',FANART,'')
	addDir3('[COLOR aqua]Top 350 Episodes[/COLOR]','http://watchseries-online.pl/last-350-episodes',54,'https://archive.org/download/TVDaily2/TVDaily2.png',FANART,'')
...
def INDEX350(url):
	...
	if '/index' in url:
		regex = re.compile('href="https://watchseries-online.pl/category/(.+?)">(.+?)</a></li><li><a').findall(open)
		for url,name in regex:
			if not '#' in name:
				addDir3(name,'https://watchseries-online.pl/category/'+url,54,icon,fanart,'')
	
	elif '/category/' in url:
		xbmc.log(str(open))
		all  = re.compile("\nhref='(.+?)' target=.+?class='listEpisode'><span.+?</span>(.+?)</a>",re.MULTILINE|re.DOTALL).findall(open)
		for url,name in all:
			addDir3(str(name).replace("&amp;","&").replace('&#39;',"'").replace('&quot;','"').replace('&#39;',"'").replace('&#8211;',' - ').replace('&#8217;',"'").replace('&#8216;',"'").replace('&#038;','&').replace('&acirc;',''),url,55,icon,fanart,'')
			
			
	elif 'movie-index' in url:
		all  = regex_get_all(open,"<li><strong>","</a>")
		for match in all:
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(@ndy builds,New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('@ndy builds Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('@ndy builds Searching Please wait',' ')
