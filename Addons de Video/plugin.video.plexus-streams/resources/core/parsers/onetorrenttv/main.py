...
def onetorrent_main():
	...
	for catid, lista in canais:
		...
		catResult[catMap[catid]] = [(name, link, img) for img,link,name in individual]
...
def list_category(categoryData):
	...
	for stream in category:
		addDir(stream[0],base_url+stream[1],401,base_url + stream[2],2,False,parser="onetorrenttv",parserfunction='play_torrent')
