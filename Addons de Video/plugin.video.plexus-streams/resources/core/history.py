...
from favourites import getUrlTypeID
	...
	if xbmcvfs.exists(history_file):
		...
		for line in lines:
			...
			if i < int(settings.getSetting('items_per_page')):
				try:
					addDir(info[0], info[1], getUrlTypeID(info[1]), info[2].translate(None,"\n"), 1, False)
				except: pass
			i+=1
	else:
		sys.exit(0)

def add_to_history(name,url,iconimage):
	line = str(name).translate(None, "|") + '|' + str(url) + '|' + str(iconimage) + '\n'
	if xbmcvfs.exists(history_file):
		lines = open(history_file).readlines()
