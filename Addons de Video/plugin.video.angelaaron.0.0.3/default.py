...
mysettings = xbmcaddon.Addon(id = 'plugin.video.angelaaron')
...
def main():
	add_dir('[B]<<<  BUSCAR  >>>[/B]', 'searchlink', 99, icon, fanart)
	if len(online_m3u) > 0:	
		add_dir('[COLOR yellow][B]>> M3U ONLINE <<[/B][/COLOR]', u_tube, 2, icon, fanart)
	if len(local_m3u) > 0:	
		add_dir('[COLOR magenta][B]>> M3U LOCAL <<[/B][/COLOR]', u_tube, 3, icon, fanart)
	if len(online_xml) > 0:	
		add_dir('[COLOR cyan][B]>>  XML ONLINE <<[/B][/COLOR]', u_tube, 4, icon, fanart)
	if len(local_xml) > 0:	
		add_dir('[COLOR lime][B]>> XML LOCAL <<[/B][/COLOR]', u_tube, 5, icon, fanart)		
...
def search(): 	
	try:
		keyb = xbmc.Keyboard('', 'Texto a Buscar')
		keyb.doModal()
		if (keyb.isConfirmed()):
