...
def MENU():
    addDir('[B][COLOR cornflowerblue]Trending[/COLOR][/B]',BASEURL + 'trending/',5,ART + 'trend.jpg',FANART,'')
    ...
    addDir('[B][COLOR cornflowerblue]Genres[/COLOR][/B]',BASEURL,3,ART + 'genres.jpg',FANART,'')
    addDir('[B][COLOR cornflowerblue]Release Year[/COLOR][/B]',BASEURL,4,ART + 'release.jpg',FANART,'')
...
def Get_Genres(url):
    OPEN = open_url(url).content
    Regex = re.compile('>Genres<(.+?)</ul>',re.DOTALL).findall(OPEN)
...
def Get_imdb(url):
    OPEN = open_url(url).content
    Regex = re.compile('<h3>Movies</h3>(.+?)<div class="top-imdb-list',re.DOTALL).findall(OPEN)
...
def RESOLVE(url):
    ...
    quality = ''
    OPEN = open_url(url).content
    match = re.compile('"label":"(.+?)".+?"file":"(.+?)"').findall(OPEN)
