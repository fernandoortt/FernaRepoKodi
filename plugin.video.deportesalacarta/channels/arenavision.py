...
def mainlist(item):
    ...
    # Descarga la página
    data = httptools.downloadpage(host, headers=cookies).data
    item.url = host + scrapertools.find_single_match(data, '(?i)href="([^"]+)"[^>]*>EVENTS GUIDE')
    data = httptools.downloadpage(item.url, headers=cookies).data
    data=re.sub(r"\n|\r|\t|\s{2}|&nbsp;|<br />","",data)
...
class Select(xbmcgui.WindowXMLDialog):
       ...
       def onInit(self):
           ...
           for canales, idioma in matches:
                ...
                for av in matchescanales:
                    url = host + scrapertools.find_single_match(data, '(?i)href="([^"]+)">ArenaVision\s*%s<' % av)
                    if not "S" in av:
       ...
       def onAction(self,action):
           if action.getId() == ACTION_SELECT_ITEM and not action.getId() == OPTIONS_OK:
               xbmc.executebuiltin('xbmc.PlayMedia(Stop)')
               self.list = self.getControl(6)
               selecitem=self.list.getSelectedItem()
               url = selecitem.getProperty("url")
               data = httptools.downloadpage(url, headers=cookies).data
           ...
           elif action.getId() == ACTION_PREVIOUS_MENU or action.getId() == ACTION_GESTURE_SWIPE_LEFT or action == 110 or action == 92 :
                global select
